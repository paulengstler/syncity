...
class CameraControls extends Script {
    ...
    _move(dt) {
        ...
        const rotateSpeed = this.rotateSpeed * 10;
        ...
        if (this._key.right) {
            ...
            //this.entity.eulers.y -= 1 * rotateSpeed;
            //this.entity.rotate(0, 1 * rotateSpeed, 0);
        ...
        const speed = this.moveSpeed; //this._key.crouch ? this.moveSlowSpeed : this._key.sprint ? this.moveFastSpeed : this.moveSpeed;
        ...
        */
        ...
        // Get the current camera (eye) position and current focus point ("at").
        const eye = this.entity.getPosition(); // eye
        // Clone the current focus (_origin) to work with it.
        const currentFocus = this._origin.clone();
        
        // Calculate the view vector from the eye to the focus.
        const viewVec = new Vec3();
        viewVec.sub2(currentFocus, eye);
        
        // We'll compute a new target focus.
        const newFocus = currentFocus.clone();

        if (this._key.left || this._key.right) {
            console.log("eye", eye, "currentFocus", currentFocus);

            // The right direction is simply the normalized view vector rotated 90 degrees around the up vector.
            const forward = viewVec.clone().normalize();
            console.log(forward, this.entity.up);
            const right = new Vec3().cross(forward, this.entity.up).normalize();
            const scaledRight = right.clone().mulScalar(this.moveSpeed/16 * this.sceneSize * dt);

            if (this._key.left) {
                newFocus.sub(scaledRight);
            }
            if (this._key.right) {
                newFocus.add(scaledRight);
            }
            this.focus(newFocus, eye, false);
        }
        
        // --- Movement: move forward/backward ---
        // Here we want to move in the camera's forward direction (derived from the view vector).
        if (this._key.forward || this._key.backward) {
            // The forward direction is simply the normalized view vector.
            const forward = viewVec.clone().normalize();
            const scaledForward = forward.clone().mulScalar(this.moveSpeed * this.sceneSize * dt);

            if (this._key.forward) {
                eye.add(scaledForward);
                newFocus.add(scaledForward);
            }
            if (this._key.backward) {
                eye.sub(scaledForward);
                newFocus.sub(scaledForward);
            }

            this.focus(newFocus, eye, false);
        }

        if (this._key.up || this._key.down) {
            // The up direction is simply the camera's up vector.
            const up = this.entity.up.clone().normalize();
            const scaledUp = up.clone().mulScalar(this.moveSpeed * this.sceneSize * dt);

            if (this._key.up) {
                eye.add(scaledUp);
                newFocus.add(scaledUp);
            }
            if (this._key.down) {
                eye.sub(scaledUp);
                newFocus.sub(scaledUp);
            }

            this.focus(newFocus, eye, false);
        }
        
        // Now, instead of fiddling with transforms directly, we delegate the update to the focus method.
        // It will update the internal _origin (the "at") and compute the camera's "eye" based on _zoomDist.

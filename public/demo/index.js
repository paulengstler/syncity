import { Asset, EnvLighting, TEXTURETYPE_DEFAULT,TEXTURETYPE_RGBP, PIXELFORMAT_RGBA8, BoundingBox, Color, Mat4, Script, Vec3, MiniStats } from 'playcanvas';
...
function initSkyboxFromTextureNew(app, env) {
...
function initSkyboxFromTexture(app, skybox) {
    if (EnvLighting) {
        return initSkyboxFromTextureNew(app, skybox);
    }
    ...
    */
    ...
    if (assets.length > 0) {
        ...
        asset.on('progress', (received, length) => {
            ...
            loadingBar.style.backgroundImage = 'linear-gradient(90deg, #000 0%, #000 ' + v + '%, white ' + v + '%, white 100%)';
            poster?.progress(v);

            if (v > 50) {
                const skybox_asset = new Asset(
                    'skybox-env-atlas',
                    'texture',
                    { url: `skybox.jpg` }
                );
                ...
                skybox_asset.ready(() => {
                    const texture = skybox_asset.resource;
                    if (texture.type === TEXTURETYPE_DEFAULT && texture.format === PIXELFORMAT_RGBA8) {
                        // assume RGBA data (pngs) are RGBM
                        texture.type = TEXTURETYPE_RGBP;
                    }
                    console.log(texture);
                    initSkyboxFromTexture(app, texture);
                });
    
                app.assets.load(skybox_asset);
            }
        });
    }
    ...
    const dom = ['arMode', 'vrMode', 'enterFullscreen', 'exitFullscreen', 'info', 'buttonContainer'].reduce((acc, id) => {

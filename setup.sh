...
if [ "$NEW_ENV" = true ] ; then
    conda create -n syncity python=3.10
    conda activate syncity
...
if [ "$BASIC" = true ] ; then
    pip install pillow imageio imageio-ffmpeg tqdm easydict opencv-python-headless scipy ninja rembg onnxruntime trimesh xatlas pyvista pymeshfix igraph transformers gsplat gradio_client lpips plyfile dill "huggingface_hub[cli]"
